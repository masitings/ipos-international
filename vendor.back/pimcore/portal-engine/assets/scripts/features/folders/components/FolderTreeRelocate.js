...
import FolderTreeNode from "~portal-engine/scripts/features/folders/components/FolderTreeNodeRelocate";
...
const Tree = connect(mapStateToProps)(props => {
    ...
    useEffect(function () {
        if (firstPageFetchingState === NOT_ASKED) {
            dispatch(requestFolderChildrenPage(parentPath, currentPage));
        }
    }, []);
            ...
            : (
                <ol className={`list-unstyled tree tree--level-${level}`}>
                    {paths.map((path, index) => {
                        return <li className={`tree__item`} key={path +'-'+ index}>
