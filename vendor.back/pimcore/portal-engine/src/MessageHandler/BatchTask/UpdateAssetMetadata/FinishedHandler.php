...
use Pimcore\Bundle\PortalEngineBundle\Message\BatchTask\UpdateAssetMetadata\FinishedMessage;
...
class FinishedHandler implements MessageHandlerInterface
{
    ...
    public function __invoke(FinishedMessage $message)
    {
        $this->logger->notice(
            sprintf(
                '[Task ID %s] Finish update asset metadata [current memory usage: %s]',
                $message->getTaskId(),
