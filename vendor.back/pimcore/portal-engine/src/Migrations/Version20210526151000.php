...
class Version20210526151000 extends AbstractPimcoreMigration
{
    ...
    protected function createIndexBasedOnTemplate(Client $esClient, $templateIndexName, $newIndexName)
    {
        ...
        if ($esClient->indices()->exists(['index' => $newIndexName])) {
            $esClient->indices()->delete(['index' => $newIndexName]);
        }
    ...
    protected function migrateIndex(string $indexName, Client $esClient, bool $updateClassDefinitionAlias = false)
    {
        ...
        if ($existsAlias) {
            $this->writeMessage($indexName . ' already a alias, do nothing.');

            return;
        }

        if ($exists) {
            $this->writeMessage('Migrate ' . $indexName);
            ...
            $this->writeMessage($indexName . ' does not exist, do nothing.');
        }
    }
    ...
    {
        $this->writeMessage('Migrating ES indexes to new naming schema...');
    ...
    protected function unMigrateIndex(string $indexName, Client $esClient)
    {
        ...
        if ($existsOdd) {
            ...
            $this->writeMessage('Index ' . $indexName . ' has no fullindex. Do nothing.');
    ...
    {
        $this->writeMessage('Migrating ES indexes to old naming schema...');
