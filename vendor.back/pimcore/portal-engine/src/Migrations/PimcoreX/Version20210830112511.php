...
class Version20210830112511 extends BundleAwareMigration
{
    ...
    public function up(Schema $schema): void
    {
        if (Config::exists('portal-engine-logo')) {
            $this->write('Thumbnail `portal-engine-logo` exists, skipping...');

            return;
        }

        $this->write('Copy thumbnail `portal-engine_footer-logo` to `portal-engine-logo`.');
    ...
    public function down(Schema $schema): void
    {
        if (Config::exists('portal-engine_footer-logo')) {
            $this->write('Thumbnail `portal-engine_footer-logo` exists, skipping...');

            return;
        }

        $this->write('Copy thumbnail `portal-engine-logo` to `portal-engine_footer-logo`.');
