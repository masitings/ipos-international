...
pimcore.registerNS("pimcore.plugin.asset_metadata_class_definitions.bundle.layoutHelper");
pimcore.plugin.asset_metadata_class_definitions.bundle.layoutHelper = {
    ...
    getRecursiveLayout: function (l, noteditable, context, skipLayoutChildren, onlyLayoutChildren, dataProvider, disableLazyRendering) {
        ...
        var tabpanelCorrection = function (panel) {
            window.setTimeout(function () {
                try {
                    ...
                    if (tab && typeof tab.pimcoreLayoutCorrected == "undefined") {
                        const childContainerArray = tab.body.query(".x-autocontainer-innerCt");
                        if(childContainerArray && childContainerArray[0]) {
                            tab.setHeight(Ext.get(childContainerArray[0]).getHeight() + 50);
                            tab.pimcoreLayoutCorrected = true;
                        }
                        tab.updateLayout();
                    }
        ...
        if (l.datatype == "layout") {
            ...
            if (pimcore.plugin.asset_metadata_class_definitions.bundle.asset.metadata.layout && pimcore.plugin.asset_metadata_class_definitions.bundle.asset.metadata.layout[l.fieldtype] !== undefined) {
                var layout = new pimcore.plugin.asset_metadata_class_definitions.bundle.asset.metadata.layout[l.fieldtype](l, context);
            ...
            if (pimcore.plugin.asset_metadata_class_definitions.bundle.tags[l.fieldtype]) {
                var dLayout;
                var data;
                var metaData = null;

                try {
                    // if (typeof dataProvider.getDataForField !== "function") {
                    data = dataProvider.getDataForField(l, context);
                    // }
                } catch (e) {
                    data = null;
                    console.log(e);
                ...
                var field = new pimcore.plugin.asset_metadata_class_definitions.bundle.tags[l.fieldtype](data, l);

                let applyDefaults = false;

                field.setObject(this.asset);
                ...
                dataProvider.addToDataFields(field, l.name, context);
                ...
                dataProvider.registerChangeListener(field);
